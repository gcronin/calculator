...
bool scroll = 0;  //whether to scroll or not
int scroll_index = 0;
...
void loop()
{
  ...
  if(scroll)  // display the input number and function(s) on the top line if math has been done
  {
    if(length > 8) // if the input is too long to fit on the screen, then scroll
    {
      if(millis() - lasttime > 750)  // do this only every 750 milliseconds.
      {
        for(int i =0; i<8; i++)
        {
          lcd.setCursor(i,0);
          if(scroll_index+i < length) {lcd.print(totalarray.charAt(scroll_index+i));}
        }
        scroll_index = (scroll_index + 1)%(length-6);
        lasttime = millis();
      }
    }
  ...
  if (key != NO_KEY)
  {
    ...
    { 
      if(function > 4 && function < 9) //these functions require only one number
      {
        ...
        scroll = 1;  // display the input number and function on the top line
        lcd.setCursor(0,1);
      ...
      else if(firstnumber == 0)  // this is the first time that enter has been pressed and we have a valid function
      {
        ...
        lcd.print("        ");  //clear line
        lcd.setCursor(0,1);
        lcd.print(totalarray);  //clear first line to make space for the second number
      ...
      else
      {
        ...
        length = length + 2;
        scroll = 1;  // display the input numbers and function on the top line
        lcd.setCursor(0,1);
        lcd.print("        ");  //clear line
        lcd.setCursor(0,1);
        printresponse(response);
    ...
    {
      if(lcdcolumnindexrow1 < 8) //buffer overrun prevention
      {
        if(key == '#' && positive ==1) //negative number read and number is currently positive
        {
          input[0] = '-';
          lcd.setCursor(0,0);
        ...
        else //read in a number or a decimal
        {
